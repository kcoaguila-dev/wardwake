...
test.describe('Wardwake Game E2E Tests', () => {
  ...
  test('Clicking [END TURN] immediately advances phase', async ({ page }) => {
    ...
    // Click [END TURN] which is located in the top HUD on the right side
    // Position approx x=285 (setOrigin(1,0)) -> x: 260-285, y: 12
    await canvas.click({ position: { x: 260, y: 18 } });
    await page.waitForTimeout(500);
